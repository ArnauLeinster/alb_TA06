...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		while (!opcio.equals("4")) {
			opcio = JOptionPane.showInputDialog(
					"Pulse 1 para comprar,2 para buscar todo, 3 para buscar algo especifico, o 4 para pagar:");
			if (opcio.equals("1")) {
	...
	private static Hashtable<String, Double> comprar(Hashtable<String, Double> almacen,
		...
		if (almacen.containsKey(com)) {
			...
			compras = (int) (compras + can);
			;
			total = total + ((can * almacen.get(com)) * iva) + (can * almacen.get(com));
		}
		return carrito;
	}
	...
	private static void buscar(Hashtable<String, Double> almacen, String busc) {
		// TODO Auto-generated method stub
		System.out.println("Producto: " + busc + "  Precio: " + almacen.get(busc) + "  Precio con IVA: "
				+ (almacen.get(busc) + (almacen.get(busc) * iva)));
	}
	...
	private static void mostrar(Hashtable<String, Double> almacen) {
		...
		while (productos.hasMoreElements()) {
			...
			System.out.println(
					"  Precio total del producto: " + precReal + "  Precio con Iva: " + (precReal + (precReal * iva)));
